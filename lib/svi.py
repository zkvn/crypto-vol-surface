...
class SVIModel(object):
    ...
    def _no_arbitrage(self):
        ...
        if self.no_butterfly:
            ineq_cons.append({"type": "ineq", "fun": self.svi_butterfly, "jac": self.svi_butterfly_jac})
...
class SVIPlot(SVIModel):
    ...
    def allsmiles(self, sv: type(SVIModel)):
        ...
        fig.update_scenes(xaxis_title_text="Log Moneyness", yaxis_title_text="Time", zaxis_title_text="Implied Volatility")
    ...
    def _change_camera(fig: go.Figure()) -> go.Figure():
        ...
        fig.update_layout(
            ...
            autosize=False,
            scene=dict(
                camera=dict(up=dict(x=0, y=0, z=1), center=dict(x=0, y=0, z=0), eye=dict(x=1.25, y=-1, z=0.25)),
